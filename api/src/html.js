...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .card { background: var(--white); border: 1px solid var(--gray-200); border-radius: 8px; padding: 20px; box-shadow: var(--shadow); transition: all 0.2s ease; }
  ...
  .panel { background: var(--white); border: 1px solid var(--gray-200); border-radius: 8px; box-shadow: var(--shadow); margin-bottom: 20px; overflow: hidden; }
  ...
  .form-input, .form-textarea { width: 100%; padding: 10px 12px; border: 1px solid var(--gray-300); border-radius: 6px; font-family: var(--font); font-size: 13px; color: var(--gray-900); background: var(--white); transition: all 0.2s ease; }
  ...
  .breathing-circle { width: 180px; height: 180px; border-radius: 50%; background: linear-gradient(135deg, var(--primary-light), var(--primary)); margin: 30px auto; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 700; color: var(--white); animation: breathe-scale 8s ease-in-out infinite; box-shadow: var(--shadow-lg); touch-action: manipulation; }
  ...
  .grounding-item { padding: 16px; background: var(--gray-50); border: 2px solid var(--gray-200); border-radius: 8px; text-align: center; cursor: pointer; transition: all 0.2s ease; user-select: none; }
  ...
  .dopamine-item { padding: 12px; background: linear-gradient(135deg, #f59e0b, #f97316); color: var(--white); border-radius: 8px; text-align: center; cursor: pointer; transition: all 0.2s ease; font-weight: 600; font-size: 12px; box-shadow: var(--shadow); user-select: none; touch-action: manipulation; }
  ...
  .modal { display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; animation: fadeIn 0.2s ease; }
  .modal.active { display: flex; }
  @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
  .modal-content { background: var(--white); border-radius: 8px; box-shadow: var(--shadow-lg); width: 90%; max-width: 500px; max-height: 80vh; overflow-y: auto; animation: slideUp 0.3s ease; }
  ...
  .close-btn { background: none; border: none; font-size: 24px; color: var(--gray-500); cursor: pointer; padding: 0; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 4px; transition: all 0.2s ease; }
  ...
  .toast { position: fixed; bottom: 20px; right: 20px; background: var(--white); border-left: 4px solid var(--primary); border-radius: 6px; padding: 16px 20px; box-shadow: var(--shadow-lg); z-index: 2000; animation: slideInRight 0.3s ease; max-width: 320px; }
  @keyframes slideInRight { from { transform: translateX(400px); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
  .toast.success { border-left-color: var(--success); }
  .toast.error { border-left-color: var(--danger); }
  .toast.warning { border-left-color: var(--warning); }
  .toast-message { font-size: 13px; color: var(--gray-900); font-weight: 500; }
  .toast-close { position: absolute; top: 8px; right: 8px; background: none; border: none; color: var(--gray-400); cursor: pointer; font-size: 16px; padding: 0; width: 24px; height: 24px; }
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      <div class="form-group">
        ...
        <input type="email" id="email" class="form-input" placeholder="you@example.com" aria-label="Email address">
        <div class="form-error" id="emailError" style="display:none;"></div>
      </div>
      <div class="form-group">
        <label class="form-label required" for="password">Password</label>
        <input type="password" id="password" class="form-input" placeholder="••••••••" aria-label="Password">
        <div class="form-error" id="passwordError" style="display:none;"></div>
      </div>
      <button class="btn btn-primary btn-lg btn-block" id="authBtn2" onclick="handleAuth()">Sign In</button>
...
<script>
  ...
  async function handleAuth() {
    ...
    // Demo auth (will replace with API)
    if (authMode === 'signin') {
  ...
  function updateTaskGrid() {
    ...
    else if (difficulty > 7 && interest > 7) advice = '💪 Challenging & engaging - Push yourself!';
    else if (difficulty < 4 && interest < 4) advice = '😴 Easy but boring - Mix with something interesting';
    else if (interest < 5) advice = '🎯 Focus on higher interest tasks';
    else if (difficulty > 8) advice = '🔥 Very hard - Break into smaller steps';
